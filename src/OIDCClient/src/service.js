...
class Service extends StateInterface{
  ...
  construct(requestArgs, params) {
    ...
    this.msgType.claims = Object.assign(args, this.msgType.claims);
  ...
  getAuthHeader(request, authnMethod, params){
    ...
    if (authnMethod){
      let hArg = this.initAuthenticationMethod(request, authnMethod, params);
      if (hArg['headers']){
   ...
   * @param {string} bodyType Which serialization to use for the HTTP body
   * @param {string} method HTTP method used
   * @param {string} authnMethod One of the six client authentication methods 
   * : bearer_body, bearer_header, client_secret_basic, client_secret_jwt, client_secret_post, private_key_jwt 
   * @param {Object.<string, string>} requestArgs Message arguments
  ...
  parseResponse(info, sformat, state, params) {
    ...
    if (resp && Object.keys(resp).indexOf('error') !== -1 &&
      ...
      if (resp && Object.keys(resp.claims).indexOf('scope') !== -1) {
        if (params['scope']){
 ...
 * @param {ServiceContext} serviceContext Contains information that a client needs to be able to talk to a server
 * @param {DB} stateDb DB class instance
 * @param {string} clientAuthnMethod One of the six client authentication methods : bearer_body, bearer_header,
