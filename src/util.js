 ...
 * :return:
 */
Util.prototype.getOrPost = function(uri, method, req, contentType, accept, kwargs){
    contentType = contentType || DEFAULT_POST_CONTENT_TYPE;
    accept = accept || null;
    var resp = {};
    var reqActions = ["GET", "DELETE"];
    var respActions = ['POST', 'PUT'];
    if (reqActions.indexOf(method)){
        if (req.keys()){
            var reqCpy = req;
            var comp = urlSplit(uri.toString());
            if (comp.query){
                reqCpy.update(this.parseQs(comp.query))
            }
            var query = reqCpy.toUrlEncoded().toString();
            resp['uri'] = this.urlunsplit((comp.scheme, comp.netloc, comp.path, query, comp.fragment));
        }else{
            resp['uri'] = uri;
        }
    }else if (respActions.indexOf(method)){
        resp['uri'] = uri;
...
Util.prototype.setCookie = function(cookieJar, kaka){
    for (var i = 0; i < Object.keys(kaka); i++){
        ...
        if (morsel['max-age'] === 0){
            try{
                this.cookieJar.clear(std_attr["domain"], std_attr["path"], std_attr["name"]);
            }catch(err){
                console.log(err)
            }
        }else{
...
Util.prototype.sortSignAlg = function(alg1, alg2){
    if (SORT_ORDER(alg1.substring(0, 2)) < SORT_ORDER[alg2.substring(0, 2)]){
        return -1;
    }else if (SORT_ORDER(alg1.substring(0, 2)) < SORT_ORDER[alg2.substring(0, 2)]){
        return 1;
    }else{
        if (alg1 < alg2){
            return -1;
