...
addRedirectUris(requestArgs, service, params){
  ...
  if (Object.keys(requestArgs).indexOf('redirect_uris') === -1){
    if (context.callback){
      let values = [];
      for (var i = 0; i < Object.keys(context.callback).length; i++){
        let key = Object.keys(context.callback)[i];
        values.append(context.callback[key]);
      }
      requestArgs['redirect_uris'] = values;
    }else{
