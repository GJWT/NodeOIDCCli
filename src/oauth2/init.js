...
var DEFAULT_SERVICES = ['Authorization', 'AccessToken',
...
function buildServices(srvs, serviceFactory, http, keyJar, clientAuthnMethod){
    var service = {};
    for (var i = 0; i < srvs.length; i++){
        var serv = srvs[i];
        var srv = serviceFactory(serv, http, keyJar, clientAuthnMethod);
        service[srv.request] = srv;
        service['any'] = new Service(http, keyJar, clientAuthnMethod);
    }
    return service;    
};
 ...
 */
Client.prototype.init = function(clientAuthnMethod, config, caCerts, keyjar, verifySsl, clientCert, httpLib, services, serviceFactory){
    ...
    this.events = null;
    this.clientInfo = new ClientInfo(keyJar, config);
    ...
    this.service = buildServices(srvs, this.serviceFactory, this.http, keyJar, cam);
    this.clientInfo.service = this.service;
    this.verifySsl = verifySsl;
};

Client.prototype.construct = function(requestType, requestArgs, extraArgs, kwargs){
...
Client.prototype.doRequest = function(requestType, scope, responseBodyType, method, requestArgs, extraArgs, httpArgs, authnMethod, kwargs){
    scope = scope || "";
    ...
    var info = srv.doRequestInit(this.clientInfo, method, scope, requestArgs, extraArgs, authnMethod, httpArgs, kwargs);
    ...
    return srv.serviceRequest(info['uri'], method, body, requestBodyType, info['httpArgs'], this.clientInfo, kwargs);
};
